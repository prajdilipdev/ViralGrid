    ...
    <div className="min-h-screen bg-[#050505] text-white flex flex-col md:flex-row">
      <aside className="md:w-60 md:min-h-screen max-w-full border-b md:border-b-0 md:border-r border-white/10 flex md:flex-col bg-[#0A0A0B] shrink-0">
        ...
        <nav className="flex md:flex-col flex-1 min-w-0 overflow-x-auto md:overflow-visible p-2 md:p-3 gap-1">
          {NAV.map(({ to, label, icon: Icon, testid }) => (
            <NavLink
              ...
              data-testid={testid}
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-2.5 rounded-md text-sm whitespace-nowrap transition-colors duration-200 ${
        ...
        <div className="hidden md:flex items-center gap-3 px-4 py-4 border-t border-white/10">
          ...
          <button data-testid="logout-button" onClick={logout} className="text-white/40 hover:text-white transition-colors duration-200">
            <LogOut size={15} />
          </button>
        </div>
      </aside>
